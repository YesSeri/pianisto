...
export default async function setupPiano(notesState: NotesState, checkboxState: CheckboxState) {
  ...
  function drawShowKeysText(x: number, y: number, key: string, group: Element | null) {
    if (!group) return;
    ...
    notes.forEach((note: any, i: number) => {
      ...
      if (showKeys !== showNotes) {
        (showKeys
          ...
          : (y: number) => drawBlackText(x + Math.round(offset / 2), y, note))(150);
      } else if (showKeys) {
  ...
  notesState.subscribe(adjustSize);
  checkboxState.subscribe((showNotes, showKeys) => draw(notesState.notes, showNotes, showKeys));
  [touchable, mouseable, keyable].forEach((xAble) => xAble(pianoSvg, pressKey, releaseKey));
}
