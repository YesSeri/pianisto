...
export default function settings(
  ...
  const showNotesCheckbox = document.getElementById(
    "showNotes"
  ) as HTMLInputElement;
  const showKeybindingsCheckbox = document.getElementById(
    "showKeybindings"
  ) as HTMLInputElement;
  ...
  function refreshOptions() {
    ...
    populateSelect(
      lowestSelect,
      allNotes.slice(0, allNotes.indexOf(highestNote))
    );
    populateSelect(
      highestSelect,
      allNotes.slice(allNotes.indexOf(lowestNote) + 1)
    );
